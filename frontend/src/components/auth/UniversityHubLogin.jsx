      ...
      <section className="role-login-modal" style={{ '--login-accent': '#7029f4', '--login-soft': '#f1eaff' }} onClick={(event) => event.stopPropagation()} aria-label="University Hub Login">
        ...
        <form
          ...
          onSubmit={(event) => {
            ...
            onLogin('university', 'University Hub Login', { ...form, mode: 'login', role: 'university' });
          }}
        >
          <label><span>University Email / ID</span><div className="role-login-input"><Mail size={15} /><input type="email" placeholder="University Email / ID" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} required /></div></label>
